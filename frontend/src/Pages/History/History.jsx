...
// Parse AI text into structured hotel cards
function parseHotels(text) {
  ...
  return blocks.filter(b => b.trim()).map((block) => {
    ...
    const lines = block.split('\n')
      ...
      .filter(l => l && !l.match(/^\d+\.\s/) && l.length > 4);
    return { name, rating, ratingMax, lines };
  }).filter(Boolean);
}

function TextHotelCard({ hotel }) {
  const scoreColor = hotel.rating >= 9 ? '#1B5E20' : hotel.rating >= 8 ? '#2E7D32' : '#1565C0';
  return (
    <div className={s.textCard}>
      <div className={s.textCardHead}>
        <p className={s.textCardName}>{hotel.name}</p>
        {hotel.rating && (
          <span className={s.textCardScore} style={{background: scoreColor}}>
            {hotel.rating}/{hotel.ratingMax}
          </span>
        )}
      </div>
      <ul className={s.textCardLines}>
          ...
          {selected && !loadingDetail && (
            <div className={s.detailContent}>
              ...
              {hotels.length > 0 ? (
                ...
                <>
                  {(() => {
                    const parsed = parseHotels(selected.analysis);
                    if (parsed.length >= 2) {
                      return (
                        <>
                          <p className={s.sectionLabel}>Recommended Hotels</p>
                          <div className={s.textGrid}>
                            {parsed.map((h, i) => <TextHotelCard key={i} hotel={h}/>)}
                          </div>
                        </>
                      );
                    }
